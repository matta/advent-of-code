...
;;
;; I refused to resort to an external regexp package, so most of this is
;; parsing.  Parsing when limited standard Common Lisp is surprisingly
;; cumbersome.

(declaim (optimize (debug 3)))

(in-package :cl-user)
(defpackage :day2b (:use #:common-lisp))
(in-package :day2b)
...
(defun must-parse-dash ()
  (must-parse-char #\-))

(defun must-parse-space ()
  (must-parse-char #\Space))

(defun must-parse-newline ()
  (must-parse-char #\Newline))

(defun parse-password (line)
  (with-input-from-string (*standard-input* line)
    (let ((low) (high) (char) (password))
      (setf low (parse-int))
      (must-parse-char #\-)
      (setf high (parse-int))
      (must-parse-char #\Space)
...
(defun parse-passwords-from-string (data)
  (with-input-from-string (*standard-input* data)
    (loop for line = (read-line *standard-input* nil)
          while line append (list (parse-password line)))))

(defun password-valid-p (password)
  (let* ((pos1 (first password))
         (pos2 (second password))
         (char (third password))
         (phrase (fourth password))
         (match1 (eql char (aref phrase (1- pos1))))
         (match2 (eql char (aref phrase (1- pos2)))))
    (and (or match1 match2)
         (not (and match1 match2)))))

(defun count-valid-passwords-from-list (passwords)
  (count-if #'password-valid-p passwords))

(defun count-valid-passwords-from-string (data)
  (count-valid-passwords-from-list
   (parse-passwords-from-string data)))

(defparameter *day2-input-example* "1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc")

(defparameter *day2-input* "13-14 f: ffffffffnfffvv
