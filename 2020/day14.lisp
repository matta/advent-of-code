;;
;; Day 14 of Advent of Code 2020, in Common Lisp.  See
;; https://adventofcode.com/2020/day/14 for the problem statement.
;;
;; I'm using only standard Common Lisp functions; no external packages.
;;

(defpackage :day14 (:use #:common-lisp))
(in-package :day14)
...
(defun starts-with-p (prefix str)
  (if (and (<= (length prefix)
               (length str))
           (search prefix str :end2 (length prefix)))
      t nil))

(defun prefix-end (prefix str)
  (unless (starts-with-p prefix str)
    (error "String ~S does not have prefix ~S." str prefix))
  (length prefix))

(defun parse-mask (line)
  (let ((end (prefix-end "mask = " line)))
    (loop with bits-mask = 0
          with bits-ones = 0
          with bits-zeroes = 0
          for ch across (subseq line end)
          do (macrolet ((shift-bits (bits)
                          `(setf ,bits (ash ,bits 1)))
                        (set-low-bit (bits)
                          `(setf ,bits (logior ,bits 1))))
               (shift-bits bits-mask)
               (shift-bits bits-ones)
               (shift-bits bits-zeroes)
               (ecase ch
                 (#\X (set-low-bit bits-mask))
                 (#\1 (set-low-bit bits-ones))
                 (#\0 (set-low-bit bits-zeroes))))
          finally (return (list 'mask
                                'mask bits-mask
                                'ones bits-ones
                                'zeroes bits-zeroes)))))

(defun parse-mem (line)
  (multiple-value-bind (address pos) (parse-integer
                                      line
                                      :start (prefix-end "mem[" line)
                                      :junk-allowed t)
    (setf line (subseq line pos))
    (setf line (subseq line (prefix-end "] = " line)))
    (let ((value (parse-integer line)))
      (list 'mem address value))))

(defun parse-instruction (line)
  (cond
    ((starts-with-p "mask" line)
     (parse-mask line))
    ((starts-with-p "mem" line)
     (parse-mem line))
    (t (error "Malformed input line: ~S" line))))
...
(defun phase-one-execute-program (program)
  (let ((*print-radix* t)
        (*print-base* 16)
        mask
        mask-ones
    ...
    (loop for instruction in program
          ...
          do (ecase op
               ...
               (mem
                (destructuring-bind (address value) args
                  (let ((masked-value (logior mask-ones
                                              (logand mask value))))
                    (setf (gethash address memory) masked-value))))))
    (loop for value being each hash-value of memory
          sum value)))

(defun phase-one (input)
  (phase-one-execute-program
   (parse-program input)))

(defun count-set-bits (number)
  (do ((count 0 (incf count)))
      ((zerop number) count)
    (setf number (logand number (1- number)))))

(defun bit-indices (number)
  (loop for index from 0
        for mask = 1 then (ash mask 1)
        until (> mask number)
        unless (zerop (logand mask number))
          collect index))

(defun project-bits (number bit-indices)
  (loop with result = 0
        for dest-index in bit-indices
        for source-index from 0
        if (logbitp source-index number)
...
(defun phase-two-execute-program (program)
  (let ((*print-radix* t)
        (*print-base* 16)
        (mask 0)
        (mask-ones 0)
               ...
               (mem
                (destructuring-bind (address-ones value) args
                  ...
                  (flet ((write-value (xaddress)
                    ...
                    (float-bits #'write-value mask))))))
    (loop for value fixnum being each hash-value of memory
          sum value)))

(defparameter *second-example-input*
...
(defun phase-two (input)
  (phase-two-execute-program
   (parse-program input)))

(defun test ()
  (assert (eql 165 (phase-one *example-input*)))
  (assert (eql 13556564111697 (phase-one *input*)))

  (assert (eql 208 (phase-two *second-example-input*)))
  (assert (eql 4173715962894 (phase-two *input*))))
