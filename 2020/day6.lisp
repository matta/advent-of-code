...
(defpackage :day6 (:use #:common-lisp))
(in-package :day6)

(declaim (optimize (debug 3)))

(defun parse-input-low (in group groups)
  (let ((line (read-line in nil 'eof)))
    (cond ((eq line 'eof)
           (if group
               ...
               groups))
          ((equal "" line)
           (parse-input-low in nil (cons group groups)))
          (t
           (parse-input-low in (cons line group) groups)))))

(defun parse-input (str)
  (with-input-from-string (in str)
    (reverse (parse-input-low in nil nil))))

(defun group-count-low (group set-fn)
  (assert (consp group))
  (let ((first (coerce (first group) 'list)))
    (if (null (rest group))
        first
        (funcall set-fn
                 first
                 (group-count-low (rest group) set-fn)))))

(defun group-count (group set-fn)
  (length (group-count-low group set-fn)))

(defun sum-group-counts (groups set-fn)
  (loop for group in groups
        sum (group-count group set-fn)))

(defun sum-group-union-counts (groups)
  (sum-group-counts groups #'union))

(defun sum-group-intersection-counts (groups)
  (sum-group-counts groups #'intersection))

(defparameter *example-input* "abc
...
(defparameter *input* "edmzkxfoprcnhijtyvl
adxntojykfcvzermplh
...
(defun test ()
  ;; *EXAMPLE-INPUT* has a group counts sum of 11.
  (assert (eql 11 (sum-group-union-counts (parse-input *example-input*))))

  ;; *INPUT* has a group counts sum of 7027.
  ;; This is the answer to Part One.
  (assert (eql 7027 (sum-group-union-counts (parse-input *input*))))

  ;; Test the intersection algorithm (Part Two) on the first group from
  ;; *EXAMPLE-INPUT*.
  (assert (eql 3 (sum-group-intersection-counts (parse-input "abc"))))

  ;; *EXAMPLE-INPUT* has a group counts sum of 11.
  (assert (eql 6 (sum-group-intersection-counts (parse-input *example-input*))))

  ;; *INPUT* has a group counts sum of ??.
  ;; This is the answer to Part One.
  (assert (eql 3579 (sum-group-intersection-counts (parse-input *input*)))))
