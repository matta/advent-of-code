...
;;;;
;;;;             I also explored "real" parsing techniques instead of
;;;;             resorting to script-like hacks, though what I came up with
;;;;             is clearly not ideal.
;;;;
;;;;             I didn't use DEFSTRUCT or DEFCLASS despite losing a lot of
;;;;             time to confusion due to the lack of typing.  Not sure if
...
(defun color-adjective-p (token)
  ;; (pushnew token *color-adjectives*)
  (member token *color-adjectives*))

(defun color-name-p (token)
  ;; (pushnew token *color-names*)
  (member token *color-names*))
...
(defun parse-color ()
  (intern
   (concatenate 'string
                (symbol-name (must-pop-token #'color-adjective-p
                                             "a color adjective"))
                "-"
                (symbol-name (must-pop-token #'color-name-p
                                             "a color name")))))
...
(defun invert-bagspec (bagspec)
  (loop :with outer-color = (first bagspec)
        :for inner :in (second bagspec)
        :collect (list (second inner) outer-color)))

(defun invert-bagspecs (bagspecs)
  (loop :with result
        :for spec :in bagspecs
        :do (loop :for inverted :in (invert-bagspec spec)
                  :for inner = (first inverted)
                  :for outer = (second inverted)
                  :for found = (assoc inner result)
                  :if found
                    :do (push outer (second found))
                  :else
                    :do (push (list inner (list outer)) result))
        :finally (return result)))

(defun find-outer-bags (color inverted-bagspecs)
  (second (assoc color inverted-bagspecs)))

(defun count-shiny-possibilities (bagspecs)
  (loop :with inverted-bagspecs = (invert-bagspecs bagspecs)
        :with frontier = (find-outer-bags 'shiny-gold inverted-bagspecs)
        :with seen = nil
        :while (not (endp frontier))
        :for color = (pop frontier)
        :do (loop :initially (push color seen)
                  :for new-color in (set-difference
                                     (set-difference
                                      (find-outer-bags color
                                                       inverted-bagspecs)
                                      frontier)
                                     seen)
                  :do (pushnew new-color frontier))
        :finally (return seen)))
...
(defun test ()
  ...
  (assert (equal '((LIGHT-RED ((1 BRIGHT-WHITE) (2 MUTED-YELLOW))))
                 (parse-document
  ...
  (assert (eql 4 (length (count-shiny-possibilities
                          (parse-document
                           (tokenize *example-input*))))))

  (assert (eql 211 (length (count-shiny-possibilities
                            (parse-document
                             (tokenize *input*))))))

  (assert (eql 12414 (count-inner-bags
                      'shiny-gold
                      (parse-document
                       (tokenize *input*))))))
