...
(defun looking-at-record-separator-p (string start end)
  (if (> 2 (- end start))
      nil
      (and (eql #\Newline (aref string start))
           (eql #\Newline (aref string (1+ start))))))

(defun eat-chars (string predicate start end)
...
(defun eat-whitespace (string start end)
  (eat-chars string #'whitespacep start end))

(defun eat-non-whitespace (string start end)
  (eat-chars string #'(lambda (ch) (not (whitespacep ch))) start end))

(defun eat-alpha (string start end)
  (eat-chars string #'alpha-char-p start end))

(defun eat-colon (string start end)
  (if (and (< start end)
           (eql #\: (aref string start)))
      (1+ start)
      start))

(defun parse-field (string start end)
  (let* ((key-start (eat-whitespace string start end))
         (key-end (eat-alpha string key-start end))
         (value-start (eat-colon string key-end end))
         (value-end (eat-non-whitespace string value-start end)))
    (assert (<= key-start key-end value-start value-end))
    (assert (equal (< key-start key-end)
                   (< value-start value-end)))
    (values (list (subseq string key-start key-end)
                  (subseq string value-start value-end))
            value-end)))

(defun parse-record-low (fields string start end)
  (multiple-value-bind (field next-start) (parse-field string start end)
    (unless (equal field '("" ""))
      (setf fields (cons field fields)))
    (if (or (eql next-start end)
            (looking-at-record-separator-p string next-start end))
        (values (reverse fields) next-start)
        (parse-record-low fields string next-start end))))

(defun parse-record (string start end)
  (parse-record-low nil string start end))

(defun parse-records-low (records string start end)
  (multiple-value-bind (record next-start) (parse-record string start end)
    (if (eql start end)
        (reverse records)
        (parse-records-low (cons record records) string next-start end))))
...
"))
                   (parse-record s 0 (length s)))))

  (assert (equal '((("ecl" "gry")
                    ("pid" "860033327")
                    ("eyr" "2020")
                    ("hcl" "#fffffd")
                    ("byr" "1937")
