...
"Returns two elements from LIST that add to SUM, or NIL if none exist."
  ...
  (let ((table (make-hash-table)))
    (dolist (a list)
      (let ((b (gethash (- sum a) table)))
        (if (numberp b)
            (return (list b a))
            (setf (gethash a table) a))))))

(defun expense-report-helper (list)
  "Returns the product of two numbers in LIST that add up to 2020."
  (reduce #'* (find-sum-to 2020 list)))

(defun expense-report ()
  (expense-report-helper '(1779
                           1737
                           1537
                           1167
                           1804
